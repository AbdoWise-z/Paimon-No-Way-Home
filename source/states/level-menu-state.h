...
class LevelMenuState : public our::State{
    ...
    void onInitialize() override {
        ...
        menuMaterial->texture = our::texture_utils::loadImage("assets/textures/level-menu.png");
        ...
        buttons[0].position = {85.0f, 63.0f};
        buttons[0].size = {307.0f, 295.0f};
        buttons[0].action = [this]() {
            this->getApp()->setConfig("config/level-0.jsonc");
            this->getApp()->changeState("play");
        };

        buttons[1].position = {495.0f, 63.0f};
        buttons[1].size = {307.0f, 295.0f};
        buttons[1].action = [this]() {
            this->getApp()->setConfig("config/level-1.jsonc");
            this->getApp()->changeState("play");
        };

        buttons[2].position = {905.0f, 63.0f};
        buttons[2].size = {307.0f, 295.0f};
        buttons[2].action = [this]() {
            this->getApp()->setConfig("config/level-2.jsonc");
            this->getApp()->changeState("play");
        };

        buttons[3].position = {283.0f, 383.0f};
        buttons[3].size = {310.0f, 295.0f};
        buttons[3].action = [this]() {
            this->getApp()->setConfig("config/app.jsonc");
            this->getApp()->changeState("play");
        };

        buttons[4].position = {693.0f, 383.0f};
        buttons[4].size = {310.0f, 295.0f};
        buttons[4].action = [this]() {
            this->getApp()->setConfig("config/app.jsonc");
            this->getApp()->changeState("play");
        };

        buttons[5].position = {1230.0f, 25.0f};
        buttons[5].size = {38.0f, 32.0f};
